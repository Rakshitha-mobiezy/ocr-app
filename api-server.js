...
// Main trigger endpoint - FIRES AND FORGETS
app.post('/trigger-automation', (req, res) => {
    // Check if automation is already running
    if (isRunning) {
        return res.status(409).json({
            status: 'already_running',
            message: 'Automation is already in progress. Please wait before triggering again.'
        });
    }
    ...
    // Store parameters for result tracking
    const requestData = {
        username,
        password,
        vcNumber,
    ...
    currentProcess = exec(command, (error, stdout, stderr) => {
        ...
        if (error) {
            ...
            fs.writeFileSync('last_result.json', JSON.stringify(resultData, null, 2));
    ...
    // IMMEDIATELY return success response (don't wait)
    res.json({
        status: 'triggered',
        message: 'Automation triggered successfully. It will run in the background.',
        data: {
            username: username,
            vcNumber: vcNumber,
            triggeredAt: new Date().toISOString()
    ...
    console.log(`✅ Response sent immediately. Automation running in background...`);
});

// Status endpoint - check if automation is running
app.get('/status', (req, res) => {
    res.json({
        isRunning: isRunning,
        message: isRunning ? 'Automation is currently running in the background' : 'Automation is idle',
        lastResult: getResult()
    });
});

// Get last result (for updating table in next phase)
app.get('/result', (req, res) => {
    try {
        const resultPath = path.join(__dirname, 'last_result.json');
        if (fs.existsSync(resultPath)) {
...
// Get request data (what was triggered)
app.get('/request', (req, res) => {
    try {
        const requestPath = path.join(__dirname, 'request_data.json');
        if (fs.existsSync(requestPath)) {
            const data = fs.readFileSync(requestPath, 'utf8');
...
app.get('/', (req, res) => {
    res.json({
        service: 'Hathway Automation API',
        version: '2.0.0',
        description: 'Fire and forget automation',
        endpoints: {
            'POST /trigger-automation': 'Trigger automation (returns immediately)',
            'GET /status': 'Check if automation is running and get last result',
            'GET /result': 'Get last automation result (for table update)',
            'GET /request': 'Get last request data',
            'GET /health': 'Check API health'
        },
        example: {
            method: 'POST',
...
app.listen(PORT, () => {
    ...
    console.log(`📍 Status check: http://localhost:${PORT}/status`);
    console.log(`📍 Results: http://localhost:${PORT}/result`);
    console.log(`📍 Request data: http://localhost:${PORT}/request`);
});